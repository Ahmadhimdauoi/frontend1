        ...
        <div className="flex-shrink-0 ml-3 mt-1 hidden md:block">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-orange-600 flex items-center justify-center shadow-md">
            <svg
              ...
              <path
                fillRule="evenodd"
                d="M4.804 21.644A6.707 6.707 0 006 21.75a6.721 6.721 0 003.583-1.029c.774.182 1.584.279 2.417.279 5.322 0 9.75-3.97 9.75-9 0-5.03-4.428-9-9.75-9s-9.75 3.97-9.75 9c0 2.409 1.025 4.587 2.718 6.163.193.681-.24 2.419-.365 3.48z"
                clipRule="evenodd"
              />
            </svg>
          </div>
          ...
          <div className="markdown-content" dir="auto">
            <ReactMarkdown
              ...
              components={{
                h1: ({ node, ...props }) => (
                  <h1
                    className="text-2xl font-bold text-primary mt-6 mb-3 pb-2 border-b border-gray-100 dark:border-gray-700"
                    {...props}
                  />
                ),
                h2: ({ node, ...props }) => (
                  <h2
                    className="text-xl font-bold text-gray-800 dark:text-gray-100 mt-5 mb-2"
                    {...props}
                  />
                ),
                h3: ({ node, ...props }) => (
                  <h3
                    className="text-lg font-bold text-gray-700 dark:text-gray-200 mt-4 mb-2"
                    {...props}
                  />
                ),
                p: ({ node, ...props }) => (
                  <p
                    className="mb-3 text-gray-700 dark:text-gray-300 leading-7"
                    {...props}
                  />
                ),
                ul: ({ node, ...props }) => (
                  <ul
                    className="list-disc list-inside space-y-1 marker:text-primary mb-4 text-gray-700 dark:text-gray-300"
                    {...props}
                  />
                ),
                ol: ({ node, ...props }) => (
                  <ol
                    className="list-decimal list-inside space-y-1 marker:text-primary mb-4 text-gray-700 dark:text-gray-300"
                    {...props}
                  />
                ),
                ...
                blockquote: ({ node, ...props }) => (
                  <blockquote
                    className="border-r-4 border-primary bg-gray-50 dark:bg-gray-900/50 p-3 my-4 rounded-l-lg text-gray-600 dark:text-gray-400 italic"
                    {...props}
                  />
                ),
                a: ({ node, ...props }) => (
                  <a
                    className="text-blue-500 hover:text-blue-600 dark:text-blue-400 underline decoration-blue-300 transition-colors"
                    target="_blank"
                ...
                code: ({
                  ...
                  return !inline ? (
                    <div className="relative my-4 group/code" dir="ltr">
                      <div className="absolute right-2 top-2 text-[10px] uppercase text-gray-400 bg-gray-800/50 px-2 py-1 rounded opacity-0 group-hover/code:opacity-100 transition-opacity">
                        {match ? match[1] : "Code"}
                      </div>
                    ...
                    <code
                      className="bg-gray-100 dark:bg-gray-700 text-primary dark:text-orange-300 px-1.5 py-0.5 rounded-md text-sm font-mono border border-gray-200 dark:border-gray-600 mx-1"
                      {...props}>
                      {children}
                    </code>
                  );
                ...
                table: ({ node, ...props }) => (
                  <div className="overflow-x-auto my-6 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800/50 shadow-md">
                    <table
                      className="min-w-full divide-y divide-gray-300 dark:divide-gray-700 border-collapse"
                      {...props}
                    />
                  </div>
                ),
                ...
                tbody: ({ node, ...props }) => (
                  <tbody
                    className="bg-white dark:bg-gray-900/80 divide-y divide-gray-200 dark:divide-gray-700"
                    {...props}
                  />
                ),
                tr: ({ node, ...props }) => (
                ...
                th: ({ node, ...props }) => (
                  <th
                    className="px-4 py-3 text-right text-sm font-bold uppercase tracking-wider border-l border-gray-300 dark:border-gray-700 last:border-l-0 whitespace-nowrap bg-gray-200/50 dark:bg-gray-700/50"
                    {...props}
                  />
                ),
                td: ({ node, ...props }) => (
                  <td
                    className="px-4 py-3 text-sm text-gray-800 dark:text-gray-200 whitespace-normal break-words border-l border-gray-200 dark:border-gray-700 last:border-l-0 align-top"
                    {...props}
                  />
                ),
                strong: ({ node, ...props }) => (
...
const ChatPage: React.FC = () => {
            ...
            setMessages([
              {
                ...
                role: "assistant",
                content:
                  bot.welcomeMessage || "مرحباً بك! كيف يمكنني مساعدتك اليوم؟",
              },
            ]);
          }
        } catch (e) {
  ...
  const handleKeySubmit = async (e: React.FormEvent) => {
    ...
    const geminiRegex = /^AIzaSy[A-Za-z0-9_-]{33}$/;
    if (!geminiRegex.test(cleanKey)) {
      alert("مفتاح API غير صالح. يجب أن يبدأ بـ AIzaSy ويتكون من 39 حرفاً.");
      return;
    }
    ...
    try {
      ...
      await registerUser(cleanUsername, cleanKey);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const responseMessage = await sendChatMessage(
        botId,
        userText,
        userApiKey
      );
      ...
      if (
        err.message &&
        err.message.includes("Invalid Google Gemini API Key")
      ) {
        handleLogout();
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-100 dark:bg-gray-900 transition-colors duration-300">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-gray-600 dark:text-gray-400 font-semibold font-cairo">
            جاري التحميل...
          </p>
        </div>
      </div>
  ...
  if (!bot) {
    return (
      <div className="text-center mt-20 text-red-500 text-xl font-bold font-cairo">
        {error}
      </div>
    );
  }
  ...
  if (!hasAccess) {
    return (
      <div
        className="min-h-screen flex items-center justify-center p-4 font-cairo bg-gray-100 dark:bg-gray-900 transition-colors duration-300"
        dir="rtl">
        {/* Theme Toggle Absolute */}
        <button
          onClick={toggleTheme}
          className="absolute top-4 left-4 p-2 rounded-full bg-white dark:bg-gray-800 shadow-md text-gray-600 dark:text-yellow-400 transition-colors z-50">
          {isDarkMode ? (
        ...
        <div className="bg-white/80 dark:bg-gray-800/95 backdrop-blur-md p-8 sm:p-12 rounded-3xl w-full max-w-md transform transition-all hover:scale-[1.01] border border-primary/10 dark:border-gray-700 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-300">
              مرحباً بك في <strong>{bot.name}</strong>. يرجى إدخال بياناتك
              للمتابعة.
            </p>
          </div>
    ...
    <div className="flex flex-col h-screen bg-gray-100 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="h-20 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 px-4 md:px-8 flex items-center justify-between shadow-sm z-20">
        <div className="flex items-center">
          ...
          <div className="mr-4">
            <h2 className="text-lg font-bold text-gray-800 dark:text-white font-cairo">
              {bot.name}
            </h2>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-yellow-400 transition-colors">
            {isDarkMode ? (
      ...
      <div className="flex-grow overflow-y-auto p-4 md:p-8 space-y-6 scroll-smooth bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed dark:invert-[0.05]">
        ...
        {isSending && (
          <div className="flex justify-start animate-slide-in">
            <div className="ml-3 mt-1 hidden md:block w-8 h-8"></div>{" "}
            {/* Spacer */}
